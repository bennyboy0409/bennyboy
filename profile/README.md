<p align="center">
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/portrait-dark.svg?v=78fbe404">
  <img alt="ASCII portrait" src="assets/portrait-light.svg?v=78fbe404">
</picture>
</p>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/hero-dark.svg?v=78fbe404">
  <img alt="25 contributions in the last year" src="assets/hero-light.svg?v=78fbe404">
</picture>

<p align="center"><a href="https://github.com/bennyboy0409">github</a></p>

## about

> Web things, mostly TypeScript. Built small, shipped early, kept only if it works.

Self-taught, prototype-first. I'd rather have a rough thing running in front of someone this week than a clean thing running in front of nobody next month.

Public repos here are the ones I'm happy to have read. The rest stays private until it earns its place.

## stack

```
typescript  javascript  react  vite  tailwind  node  python  swift  html
css  git
```

## projects

**[neuro-psy](https://github.com/bennyboy0409/neuro-psy)** · `typescript`

**[hermes-showcase](https://github.com/bennyboy0409/hermes-showcase)** · `html`  
Website-Demos: Roestwerk, Ton und Form + Design-Bausteine

**[vantage-studio](https://github.com/bennyboy0409/bennyboy)** · `react, typescript`  
Two dozen hover, scroll and cursor interaction tiles in one page. A parts bin for landing pages.

**[rockie](https://rockie-energy-guide.lovable.app)** · `react, typescript`  
Guided energy-advice chat. Walks through an assessment step by step and shows the maths behind every recommendation.

## stats

```
1                          2
current streak             longest streak
jul 29 – jul 29            jul 12 – jul 13

BY BYTES                                  BY REPOS
typescript  █████████████████       76%   typescript  ██████████  2
html        █████                   23%   html        █████       1
css         █                        1%

THE YEAR
    aug  sep oct nov  dec jan  feb mar apr may  jun jul
                                                      :
mon                                                   :
                                                     :
wed                                                     #

fri
                       :

5 of 365 days had a contribution
```

<sub>generated from the GitHub API · last run 2026-07-30 14:04:15 UTC</sub>
