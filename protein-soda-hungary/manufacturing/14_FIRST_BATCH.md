...
| | |
|---|---|
| **Product** | **ONE SKU.** 330 ml aluminium can, carbonated, clear, zero sugar, **20 g protein**. **Flavour: Lemon-Lime.** |
| **Formula** | **ARCH F** — 10 g hydrolysed bovine collagen + 10 g whey protein isolate + 1.7 g L-leucine + 150 mg L-tryptophan |
| **Quantity** | **~1,700 cans** |
| **Manufacturer** | **Opre' Label** — `sales@oprelabel.com` — one-pallet MOQ, collagen beverages on their published capability list |
...
| **Landed cost** | **491 HUF/can** = **EUR 1.35** `ESTIMATE` (range 428–620) |
...
| **Total cash needed** | **~EUR 2,900–3,250 = 1,053,000–1,180,000 HUF** (production EUR 2,300 + setup/compliance ~EUR 500–900) |
...
**Not less,** because ~1,700 is what one pallet of 330 ml cans physically is. `DERIVED`: a 330 ml sleek can is ~53–58 mm diameter; a 1200×800 Euro pallet takes ~280 cans/layer × 6 layers ≈ **1,680 cans**; a 1200×1000 industrial pallet ~350–400/layer × 5–6 layers ≈ 1,750–2,400. **Range 1,500–2,500. This is arithmetic, not a quote — the first thing Opre' must confirm.**
...
At 1,700 cans you sit at **34% of that ceiling** — enough headroom for a second small run in the same year if the first sells out.
...
|---|---:|---:|---:|---:|---|
| Collagen 10 g (hydrolysed bovine) | 79.0 | 0.218 | 134,300 | 370 | ESTIMATE |
| Whey protein isolate 10 g | 118.0 | 0.325 | 200,600 | 553 | ESTIMATE ⚠️ see 2.3 |
| L-leucine 1.7 g | 9.6 | 0.026 | 16,320 | 45 | ESTIMATE |
| L-tryptophan 150 mg | 0.7 | 0.002 | 1,190 | 3 | ESTIMATE |
| Sweetener (sucralose + acesulfame K) | 6.2 | 0.017 | 10,540 | 29 | ESTIMATE |
| Acid system (citric + malic) | 2.5 | 0.007 | 4,250 | 12 | ESTIMATE |
| Natural flavour, lemon-lime | 8.0 | 0.022 | 13,600 | 37 | ESTIMATE |
| Potassium sorbate | 0.8 | 0.002 | 1,360 | 4 | ESTIMATE — **omit if tunnel-pasteurised** |
| **Subtotal ingredients** | **224.8** | **0.619** | **382,160** | **1,053** | ESTIMATE |
| Can body + end (co-packer stock) | 52.0 | 0.143 | 88,400 | 244 | **QUOTE REQUIRED** |
| Shrink sleeve + application | 42.0 | 0.116 | 71,400 | 197 | **QUOTE REQUIRED** |
| Filling / tolling, variable | 60.0 | 0.165 | 102,000 | 281 | **QUOTE REQUIRED — the largest unknown** |
| Setup / changeover / recipe fee, amortised | 70.0 | 0.193 | 119,000 | 328 | **QUOTE REQUIRED — dominates at this volume** |
| Tray / carton / film | 8.0 | 0.022 | 13,600 | 37 | ESTIMATE |
| Inbound freight to Hungary | 15.0 | 0.041 | 25,500 | 70 | ESTIMATE — **higher if Opre' fills in Slovakia** |
| EPR packaging fee | 5.1 | 0.014 | 8,670 | 24 | ESTIMATE |
| Waste / reject allowance 3% | 14.0 | 0.039 | 23,800 | 66 | ESTIMATE — first runs reject more |
| **TOTAL LANDED COGS** | **491.0** | **1.353** | **834,500** | **2,299** | **ESTIMATE, range 428–620** |

**Cross-check:** sibling dossier B6 independently modelled **EUR 2,000–2,600 for ~1,700 cans** = **428–555 HUF/can**. Our 491 sits inside that range. Two independent routes to the same number is the strongest validation available without a quote.

**Note a small internal inconsistency to reconcile:** the agreed plan quotes ingredients at **~213 HUF/can pilot**, the finance model itemises **224.8 HUF**. The gap is ~12 HUF/can (~20,000 HUF on the batch) and appears to be flavour and sweetener treated inside or outside the "ingredient" line. **Use 224.8 in the cash plan** — it is the one that sums to 491 — and reconcile when the ingredient quotes land.

### 2.2 Total cash required to get batch 1 sold

| Item | Lean EUR | Base EUR | High EUR | Base HUF | Status |
|---|---:|---:|---:|---:|---|
| **Production — 1,700 cans all-in landed** | 2,000 | **2,300** | 2,600 | **834,900** | ESTIMATE |
...
| Laboratory — nutrition analysis | 0 | **0** | 0 | 0 | **NOT REQUIRED** — see 2.4 |
...
| **SUBTOTAL** | **2,199** | **2,858** | **3,784** | **1,037,400** | |
| Contingency at 15% | 300 | **390** | 500 | 141,570 | **DO NOT SKIP** |
| **TOTAL CASH REQUIRED** | **~2,500** | **~3,250** | **~4,300** | **~1,180,000** | |

### 2.3 ⚠️ THE BLUNT VERDICT ON THE CAPITAL

**Available: EUR 1,000 personal + ~EUR 2,000 family = EUR 3,000 = 1,089,000 HUF.**
**Base case needs ~EUR 3,250. The gap is ~EUR 250 (≈91,000 HUF).**

**EUR 1,000 alone does not buy a can run. It never did.** The batch only fits inside EUR 3,000 if:

1. **artwork is self-designed** (Canva/Figma) — saves EUR 80–400;
2. **you skip the card reader** and take cash — saves EUR 39;
3. **you negotiate the classification opinion down to a scoped question** rather than a full opinion — saves ~EUR 125;
4. **the contingency is funded from the first EUR 400 of sales rather than held up front** — the riskiest of the four, and the one to avoid if you can.

**Do 1–3 and hold the contingency: EUR 3,000 covers it, just.** Do not spend the contingency on a bigger batch.

⚠️ **Two ingredient warnings that could break this budget:**
- **Whey protein isolate at 118 HUF/can is the second-largest single line and the most volatile.** European producers were reported **SOLD OUT in April 2026**, with WPC80 at EUR 20,250/t. **Get a written WPI price and availability before you commit to ARCH F.** If WPI has moved, the collagen-only fallback formula is your protection — have it costed now, not later.
- **The setup/changeover fee (70 HUF/can amortised, EUR 328 on the batch) is a QUOTE REQUIRED figure that could be double.** At 1,700 cans a fixed fee is punishing: every EUR 100 of setup fee is **21 HUF on every can.** Ask for it as a number, not a percentage.

### 2.4 Lab cost: budget 0, and here is why that is legitimate

**Nutrition analysis is NOT required. This saves EUR 800–2,000.** Reg. (EU) 1169/2011 Art. 31(4) permits nutrition declaration values to be established **by calculation from known average values of the ingredients used.** Your formula is a defined recipe of purchased ingredients with supplier specification sheets — calculate the panel from those specs. `LIKELY — confirm in the food-law classification opinion.`
...
At **799 HUF RRP** including 27% ÁFA:

| | HUF | EUR |
|---|---:|---:|
| Consumer price incl. ÁFA | **799.00** | 2.201 |
| less ÁFA at 27% (799 ÷ 1.27) | −169.87 | −0.468 |
| **Net revenue per can** | **629.13** | **1.733** |
| less landed COGS | −491.00 | −1.353 |
| **CONTRIBUTION PER CAN** | **+138.13** | **+0.380** |

**Sold direct or on consignment, you keep the retail margin. That is the only reason this is positive.**

| Scenario | Contribution/can | Verdict |
|---|---:|---|
| **Direct sale at 799 HUF** | **+138 HUF** | ✅ The plan |
| Direct sale at 899 HUF | +217 HUF | ✅ Better, if the channel bears it |
| Direct sale at 699 HUF | +59 HUF | ⚠️ Thin — see below |
| **Wholesale at 470 HUF net** | **−21 HUF** | ❌ **A LOSS ON EVERY CAN. DO NOT WHOLESALE BATCH 1.** |

> ### ⚠️ WHOLESALE IS NOT VIABLE UNTIL ~25,000 CANS
> COGS falls to ~420 HUF at 25,000 cans, the first volume at which a 470 HUF wholesale price produces a positive **+50 HUF**. At 5,000 cans COGS is ~699 HUF (−47% gross margin at wholesale). At 10,000 cans ~556 HUF (still −86 HUF). **Anyone who offers to "take a pallet at wholesale" in month one is offering to take your money. Say no politely and offer consignment instead.**

### 3.1 Break-even, stated plainly

- **Total batch cost: 834,500 HUF.**
- **Net revenue per can sold at 799: 629 HUF.**
- **BREAK-EVEN = 834,500 ÷ 629 = 1,327 cans.** `DERIVED`

Budget **~175 cans as free samples** (your main sales tool — this is deliberate, not waste). That leaves **1,525 sellable cans**, of which you must sell **1,327 = 87%** just to get your money back.

| RRP | Cans sold (of 1,525) | Net position on the batch |
|---|---|---|
| 799 HUF | all 1,525 | **+124,500 HUF (+EUR 343)** |
| 899 HUF | all 1,525 | **+244,800 HUF (+EUR 674)** |
| **699 HUF** | all 1,525 | **+4,600 HUF (+EUR 13)** — **essentially zero. This is why 699 is the floor, not the target.** |
| 799 HUF | 1,327 | **0 — break-even** |
| 799 HUF | 1,000 | **−205,500 HUF (−EUR 566)** |

**The honest read: batch 1 is a paid education, not a profit centre.** If everything goes right you end the batch with roughly your money back plus ~EUR 340 and — far more valuable — a real product, real stockists and real quotes. **Plan for that outcome, not for a windfall.**
...
|---|---|---|
| 1 | **Exact cans per pallet at 330 ml** | The whole batch size is derived arithmetic. If a pallet is 1,200 cans the economics shift; if it is 2,400 you may exceed your cash. |
...
| 9 | **Thermal process: tunnel pasteurisation, hot-fill or cold-fill + sorbate?** And the resulting ambient shelf life at pH 3.2 with protein | Decides whether potassium sorbate stays in the formula and on the label. |
| 10 | **Can internal lacquer specification** | Low pH + protein + 12-month shelf life needs the right liner. |
...
| **Ölgerðin (Iceland)** — find the contact on olgerdin.is | *"Do you contract-manufacture or white-label your carbonated collagen can for third parties? MOQ, price, EU export terms?"* | The only European producer already making this exact product. Even a "no" gives you the only real category benchmark that exists. |
...
| **Hungarian food lawyer / élelmiszerjogi tanácsadó** | One scoped opinion covering **(a) DRS scope below 5,000 units, (b) ÁFA rate 27 vs 18, (c) NETA scope for a zero-sugar protein soda** | **The highest-leverage EUR 275 in this project.** It settles the three questions that determine your artwork, your price and your tax. |
...
**Send every co-packer the same one-page spec so the quotes are comparable:** 330 ml can, carbonated 4–5 g/L CO₂, zero sugar, sucralose + acesulfame K, pH ~3.2, 80–100 kcal, **quote ARCH F (10 g collagen + 10 g WPI + 1.7 g leucine + 150 mg tryptophan) AND a collagen-only fallback separately**, **blank cans + shrink sleeve — never printed**, quantities 1,700 / 5,000 / 10,000 / 25,000, delivered Budapest.
...
- [ ] **Hungarian food-law classification opinion obtained**, settling: **DRS scope below ~5,000 units**, ÁFA rate, NETA scope.
...
- [ ] **GTIN obtained from GS1 Hungary** (not a reseller barcode).
...
- [ ] **Thermal process and resulting ambient shelf life stated in writing** for a pH 3.2 protein matrix.
...
- [ ] **Total cash confirmed ≥ EUR 2,900** (production + setup + compliance), with the **15% contingency held separately and not spent.**
- [ ] Landed cost per can from the actual quote is **≤ 550 HUF**. ⚠️ **At 629 HUF net revenue, a landed cost above 550 leaves under 80 HUF/can and the batch cannot absorb a single mistake.**
- [ ] **WPI price and availability confirmed in writing** (European producers reported sold out April 2026).
- [ ] Collagen-only fallback formula costed, so a dairy refusal does not stop the batch.
- **🛑 NO-GO IF:** landed cost exceeds **620 HUF/can** (the top of the modelled range). At that point contribution at 799 RRP is under 10 HUF and the batch cannot pay for itself. **Renegotiate, switch supplier, or drop to the powder SKU.**

### Gate 4 — DEMAND (before production, not after)
- [ ] **At least 3 gyms / studios / supplement stores have agreed in writing (email is fine) to take stock on consignment.**
- [ ] **A route to 1,327 sold cans exists on paper** — that is break-even, and it is 87% of your sellable stock after samples.
- [ ] **175 cans budgeted as free samples** and allocated to named prospects, not handed out at random.
- [ ] Price tested verbally at 799 HUF against NOCCO (690–940 HUF) and Barebells (890–999 HUF) with at least five real gym-goers.
- **🛑 NO-GO IF:** you cannot name where the first 500 cans go. **1,700 cans in a garage is not a business, it is 834,500 HUF of inventory.**
...
| If | Then |
|---|---|
| **Opre' quotes above budget or has no real one-pallet MOQ** | Go to **Bubee**. If Bubee's MOQ is 3,000–5,000 cans, take it — still under the DRS ceiling, Hungarian, 110 km. |
...
| **No filler will go below ~6,000 cans** | ❌ Do not stretch. **6,000+ cans breaks the 5,000-unit DRS exemption** and adds registration, a 45-day gate and a DRS mark to the artwork — a cost far larger than the extra cans. |
